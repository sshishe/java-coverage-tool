...
public class AntXmlRunListener extends RunListener {
	Logger logger = LoggerFactory.getLogger(AntXmlRunListener.class);
	...
	public void testFailure(Failure f) {
		logger.info("Test Failed: " + f.getDescription().getClassName()
				+ " at method: " + f.getDescription().getMethodName());
		formatError(FAILURE, f);
	}

	public void testAssumptionFailure(Failure f) {
		logger.info("Test Assumption Failed: " + f.getDescription().getClassName()
				+ " at method: " + f.getDescription().getMethodName());
		formatError(FAILURE, f);
	}
	...
	public void testFinished(Description descr) {
		logger.info("Test Finished: " + descr.getClassName() + " at method: "
				+ descr.getMethodName());
	...
	public void testRunFinished(Result result) {
		try {
			try {
				...
				m_rootElement.setAttribute(ATTR_TESTS,
						"" + result.getRunCount());
				m_rootElement.setAttribute(ATTR_FAILURES,
						"" + result.getFailureCount());
				// JUnit4 does not seem to discern between failures and errors.
				m_rootElement.setAttribute(ATTR_ERRORS, "" + 0);
				m_rootElement.setAttribute(ATTR_TIME, ""
						+ (result.getRunTime() / ONE_SECOND));
				...
				System.err.println("Exception at testRunFinished");
			} finally {
				outputStream.close();
			}
